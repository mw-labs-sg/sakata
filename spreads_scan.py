...
def render_scan_tab(is_mobile):
    ...
    pos_c = theme['pos']
    _lbl = f"color:#334155;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;font-family:{FONTS}"
    _bg3 = theme.get('bg3', '#f8fafc'); _mut = theme.get('muted', '#94a3b8')
    ann_factor = 252
...
def _render_scan_table(sorted_results, theme):
    ...
    _txt = theme.get('text', '#334155'); _txt2 = theme.get('text2', '#64748b')
    ...
    for rank, p in enumerate(sorted_results, 1):
        ...
        is_top3 = rank <= 3
        bg = f'rgba(15,118,110,0.05)' if is_top3 else 'transparent'
