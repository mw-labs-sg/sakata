...
sk_backtest.py — walk-forward behind the Portfolio tab's Rebalance control: refit on a calendar cadence over a rolling (or anchored) lookback, size each fit to the vol target, charge the fees, chain the held segments; sweeps cadence x lookback over one shared fit cache to find the pairing that holds up best | imports: sakata_stats, sk_portfolio | called by: app
